...
import { create_order, get_categories } from "../../../store/slice/adminSlice";
...
export default function CreateProductPage() {
  ...
  const { loading, error, created_order, categories } = useSelector((state) => state.admin);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    const submissionData = {
      ...
      await dispatch(create_order(submissionData));
    setSubmitting(false);
    resetForm();
    router.refresh();
        ...
        {({ isSubmitting }) => (
          <Form className="space-y-4">
            ...
            <div>
              ...
              <Field name="image_name" className="border p-2 w-full rounded" />
              <ErrorMessage name="image_name" component="div" className="text-red-500 text-sm" />
            </div>
